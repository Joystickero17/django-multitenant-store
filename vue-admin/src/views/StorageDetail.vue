    ...
    <div class="mx-5">
        ...
        <div class="row my-5 justify-content-center">
            <div class="col-6">
                ...
                <div class="mb-3">
                    <p>Dirección Actual: {{ this.actual_object.region }}, {{ this.actual_object.subregion }}<span v-if="this.actual_object.city">, {{ this.actual_object.city }}</span></p>
                </div>
                <div class="mb-3">
...
export default {
    ...
    methods: {
        ...
        saveStorage() {
            ...
            this.$axios.put(`api/product_storage/${this.$route.params.id}/`, data)
            .then((res)=>{
                console.log(res)
        ...
        getRegionFromAscii(ascii){
            return this.regions.find(item=>item.ascii_name==ascii)
        },
        getSubRegionFromAscii(ascii){
            return this.subregions.find(item=>item.ascii_name==ascii)
        },
        getCityFromAscii(ascii){
            return this.cities.find(item=>item.ascii_name==ascii)
        },
        getItem(){
            this.$axios.get(`api/product_storage/${this.$route.params.id}/`)
           .then((res)=>{
                this.storage_type = res.data.storage_type
                this.center = [res.data.coordinate_x,res.data.coordinate_y]
                this.point = [res.data.coordinate_x,res.data.coordinate_y]
                this.zoom = res.data.zoom
                this.actual_object = res.data
            })
            .catch(err=>console.log(err))
        },
        getAllData(){
